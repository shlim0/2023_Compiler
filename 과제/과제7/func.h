...
A_NODE* makeNode (NODE_NAME n, A_NODE *a, A_NODE *b, A_NODE *c) { // make new node for syntax tree
	A_NODE *m;
...
A_NODE* makeNodeList (NODE_NAME n, A_NODE *a, A_NODE *b) { // 신택스 트리를 목록의 형태로 구현
	A_NODE *m, *k;
	k = a;
	while (k -> rlink)
		k = k-> rlink;
...
A_ID *makeIdentifier (char *s) { // make a new declarator for identifier
	A_ID *id;
...
A_ID *makeDummyIdentifier () { // make a new declarator for dummy identifier
	A_ID *id;
	id = malloc(sizeof(A_ID));
	id -> name = "";
	id -> kind = 0;
...
A_TYPE *makeType (T_KIND k) { // make a new type
	A_TYPE *t;
...
A_SPECIFIER *makeSpecifier(A_TYPE *t, S_KIND s) { // make a new specifier
	A_SPECIFIER *p;
...
void setDefaultSpecifier(A_SPECIFIER *p) { // set default specifier
	A_TYPE *t;
	if (p -> type == NIL)
		p -> type = int_type;
	if (p -> stor == S_NULL)
...
A_SPECIFIER *updateSpecifier(A_SPECIFIER *p, A_TYPE *t, S_KIND s) { // merge & update specifier
	if (t)
		if (p->type)
			if (p->type == t)
				;
...
A_ID *linkDeclaratorList(A_ID *id1, A_ID *id2) { //link two declarator list id1 & id2
	A_ID *m = id1;
	if (id1 == NIL)
		return (id2);
	while (m -> link)
...
A_ID *getIdentifierDeclared(char *s) { // check if identifier is already declared in primary expression
	A_ID *id;
...
A_TYPE *getTypeOfStructOrEnumRefIdentifier(T_KIND k, char *s, ID_KIND kk) { //get type of struct identifier
	A_TYPE *t;
	A_ID *id;
	id = searchIdentifier(s, current_id);
	if (id)
...
A_ID *setDeclaratorInit (A_ID *id, A_NODE *n) { // set declarator init (expression tree)
	id -> init = n;
	return (id);
}

A_ID *setDeclaratorKind (A_ID *id, ID_KIND k) { // set declarator kind
	A_ID *a;
...
A_ID *setDeclaratorType (A_ID *id, A_TYPE *t) { // set declarator type
	id -> type = t;
	return (id);
}

A_ID *setDeclaratorElementType (A_ID *id, A_TYPE *t) { // set declarator type (or element type)
	A_TYPE *tt;
	if (id -> type == NIL)
		id -> type = t;
	else {
...
A_ID *setDeclaratorTypeAndKind (A_ID *id, A_TYPE *t, ID_KIND k) { // set declarator element type and kind
...
A_ID *setDeclaratorListSpecifier (A_ID *id, A_SPECIFIER *p) { // set declarator_list type and kind based on storage class
	A_ID *a;
	setDefaultSpecifier(p);
	a = id;
	while (a) {
...
A_ID *setParameterDeclaratorSpecifier (A_ID *id, A_SPECIFIER *p) { // set declarator _list type and kind
	// check redeclaration
	...
	if (p -> stor)
		syntax_error(14);
	setDefaultSpecifier(p);
	if(p -> type != void_type){ // void_type아 아닌 경우에만
		id = setDeclaratorElementType(id, p -> type);
		id -> kind = ID_PARM;
	}
	return (id);
...
A_TYPE *setTypeField (A_TYPE *t, A_ID *n) { // set type field
	t -> field = n;
	return (t);
}

A_TYPE *setTypeExpr (A_TYPE *t, A_NODE *n) { // set type initial value (expression tree)
	t -> expr = n;
	return (t);
}

A_TYPE *setTypeStructOrEnumIdentifier (T_KIND k, char *s, ID_KIND kk) { // set type initial value
	A_TYPE *t;
	A_ID *id, *a;
...
A_TYPE *setTypeAndKindOfDeclarator (A_TYPE *t, ID_KIND k, A_ID *id) { // set type and kind of identifier 
	if (searchIdentifierAtCurrentLevel(id -> name, id -> prev))
		syntax_error(12, id -> name);
	id -> type = t;
	id -> kind = k;
...
BOOLEAN isNotSameFormalParameters (A_ID *a, A_ID *b) { // check function parameters with protype
