...
double atof(); // literal table에서 float 값이 제대로 출력되지 않아 double로 변경
...
// func.h에서 사용하는 함수와 이름이 중복되어 link가 안되므로 접미사로 _sem을
// 붙여 구분함
...
A_TYPE *sem_expression(A_NODE *node)
{
  ...
  case N_EXP_STRUCT:
    ...
    if (id)
    {
      result = id->type;
      printf(result);
      // i.e. struct s{struct s b;}a; a.b;시 에러가 나게 된다.
      if (isStructOrUnionType(result)) // 추가된 부분
  ...
  case N_EXP_ASSIGN:
    ...
    // 7강 슬라이드에는 node->rlink가 아닌 node로 되어있음
...
int sem_statement(A_NODE *node, int addr, A_TYPE *ret, BOOLEAN sw, BOOLEAN brk,
                  BOOLEAN cnt)
{
  int local_size = 0, i;
  A_LITERAL lit;
  ...
  case N_STMT_COMPOUND:
    ...
    local_size +=
        sem_statement_list(node->rlink, local_size + addr, ret, sw, brk, cnt);
    break;
  case N_STMT_EMPTY:
    break;
...
int sem_A_TYPE(A_TYPE *t)
{
  ...
  case T_ARRAY:
    ...
    // 부록6에는 i에 대입하나 7강 강의 슬라이드에는 result에 대입함
...
int sem_declaration(A_ID *id, int addr)
{
  ...
  case ID_VAR:
    ...
    // if (id->init) // initializer
    //   sem_initializer(id->init);
...
BOOLEAN isAllowableAssignmentConversion(A_TYPE *t1, A_TYPE *t2,
                                        A_NODE *node) // t1 <--- t2
{
  if (isArithmeticType(t1) && isArithmeticType(t2))
    return (TRUE);
    ...
    return (TRUE);
  // 두 포인터 타입이 같은 경우. i.e. int * <- int *나 float * <- float *
  else if (isPointerType(t1) &&
           (isConstantZeroExp(node) || isCompatiblePointerType(t1, t2)))
    return (TRUE);
  // 두 포인터 타입이 다른 경우. i.e. int * <- float *나 char * <- int *
  else if (isPointerType(t1) && isPointerType(t2))
    semantic_warning(11, node->line);
  // integral_type <- pointer_type
  else if (isIntegralType(t1) && isPointerType(t2))
    semantic_warning(11, node->line);
...
BOOLEAN isAllowableCastingConversion(A_TYPE *t1, A_TYPE *t2)
{
  ...
  if (isAnyIntegerType(t1) &&
    ...
    return (TRUE);
  // i.e. int_type <- enum_type이 가능하도록 함.
  else if (isAnyIntegerType(t1) && (isScalarType(t2)))
    return (TRUE);
  // i.e. enum_type <- scalar_type이 가능하도록 함.
  else if (isIntegralType(t1) && (isScalarType(t2)))
    return (TRUE);
  // i.e. pointer_type <- enum_type이 가능하도록 함.
  else if (isPointerType(t1) && (isScalarType(t2)))
    return (TRUE);
  else
    return (FALSE);
}
...
{
  semantic_err++;
  printf("%s*** semantic error at line %d : ", C_RED, ll);
  switch (i)
  {
  // errors in expression
  case 13:
    printf("artih type expr required in unary operation\n");
    break;
  case 18:
  ...
  case 49:
    ...
    break;
  case 50:
    printf("intergral type expression required in statement\n");
    break;
  case 51:
  ...
  case 86:
    ...
    break;
  case 89:
    printf("unknown identifer kind : %s\n", s);
    break;
  // misc errors
  case 90:
...
void semantic_warning(int i, int ll)
{
  ...
  printf("%s--- warning at line %d :", C_YLLW, ll); // 노란색으로 warning 표시
  switch (i)
  {
  case 11:
