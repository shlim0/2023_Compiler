...
char *opcode_name[] = {
    "OP_NULL", "LOD",     "LDX",       "LDXB",    "LDA",   "LITI", "STO",
    "STOB",    "STX",     "STXB",      "SUBI",    "SUBF",  "DIVI", "DIVF",
    "ADDI",    "ADDF",    "OFFSET",    "MULI",    "MULF",  "MOD",  "LSSI",
    "LSSF",    "GTRI",    "GTRF",      "LEQI",    "LEQF",  "GEQI", "GEQF",
    "NEQI",    "NEQF",    "EQLI",      "EQLF",    "NOT",   "OR",   "AND",
    "CVTI",    "CVTF",    "JPC",       "JPCR",    "JMP",   "JPT",  "JPTR",
    "INT",     "INCI",    "INCF",      "DECI",    "DECF",  "SUP",  "CAL",
    "ADDR",    "RET",     "MINUSI",    "MINUSF",  "CHK",   "LDI",  "LDIB",
...
// macOS에서 발생하는 implict func declaration 컴파일 에러 방지
int isArrayType();
int isPointerOrArrayType();
int isFloatType();
...
int label_no = 0;
int gen_err = 0;
// 함수에서 return을 명시하면 .asm에서 RET이 두번 되므로 한번만 RET되게 사용하는
// 변수.
int is_return = 0;
...
void gen_expression(A_NODE *node) {
  ...
  case N_EXP_IDENT:
    ...
    case ID_PARM:
      ...
      case T_ARRAY:
        ...
        break;
      case T_STRUCT:
        gen_error(24, node->line, "T_STRUCT"); // 구현 범위 제외
        break;
      case T_UNION:
        gen_error(24, node->line, "T_UNION"); // 구현 범위 제외
  ...
  case N_EXP_STRUCT:
    ...
    // STRUCT은 구현 범위 제외이므로 error 처리 하였다.
    // 단, struct s{struct s* ptr;}a; a.ptr; 같은 경우,

    // if(!isPointerOrArrayType(id->type))
    // gen_error(24,node->line,"N_EXP_STRUCT");

    // 처럼 코드를 작성하여 코드 생성을 유도할 수 있다.
...
void gen_statement(A_NODE *node, int cont_label, int break_label, A_SWITCH sw[],
                   int *sn) {
  ...
  case N_STMT_LABEL_CASE:
    gen_error(24, node->line, "N_STMT_LABEL_CASE"); // 구현 범위 제외
    // if (sw) {
    // 	*sn=*sn+1;
    // 	sw[*sn].kind=SW_VALUE;
  ...
  case N_STMT_LABEL_DEFAULT:
    gen_error(24, node->line, "N_STMT_LABEL_DEFAULT"); // 구현 범위 제외
    // if (sw) {
    // 	*sn=*sn+1;
  ...
  case N_STMT_IF_ELSE:
    ...
    break;
  case N_STMT_SWITCH:
    gen_error(24, node->line, "N_STMT_SWITCH"); // 구현 범위 제외
    // gen_expression(node->llink);
...
void gen_declaration(A_ID *id) {
  ...
  case ID_FUNC:
    if (id->type->expr) {
      ...
      // 이미 return을 명시한 상태면 RET이 두번 되기 때문에 이를 조정한다.
      if (!is_return)
        gen_code_i(RET, 0, 0);
      else
        is_return = 0;
  ...
  case ID_ENUM:
    break;
  case ID_STRUCT:
    gen_error(24, node->line, "ID_STRUCT"); // 구현 범위 제외
  case ID_FIELD:
  case ID_ENUM_LITERAL:
  case ID_NULL:
    break;
...
void gen_label_number(int i) { fprintf(fout, "L%d:\n", i); }

void gen_label_name(char *s) { fprintf(fout, "%s:\n", s); }
